                ...
                <form action="{{ route('admin.categories.update') }}" method="POST" role="form" enctype="multipart/form-data">
                    ...
                    <div class="tile-body">
                        <div class="form-group">
                            ...
                            <input class="form-control @error('name') is-invalid @enderror" type="text" name="name" id="name" value="{{ old('name', $t_category->name) }}" />
                            <input type="hidden" name="id" value="{{ $t_category->id }}">
                            @error('name')<p style="color: red">{{ $message }}</p> @enderror
                        </div>

                        <div class="form-group">
                            <label class="control-label" for="short_desc">Description <span class="m-l-5 text-danger"> *</span></label>
                            <input class="form-control @error('short_desc') is-invalid @enderror" type="text" name="short_desc" id="short_desc" value="{{ old('short_desc', $t_category->short_desc) }}" />
                            <input type="hidden" name="id" value="{{ $t_category->id }}">
